...
class LabelEmbedder(nn.Module):
    ...
    """
    def __init__(self, in_size, hidden_size, dropout_prob=0.1, conditions_shape=None):
        super().__init__()
        if conditions_shape is None:
            conditions_shape = (1, 1, in_size)
        self.linear = nn.Linear(in_size, hidden_size)
        self.dropout_prob = dropout_prob
        if dropout_prob > 0:
            self.uncondition = nn.Parameter(0.02 * torch.randn(conditions_shape[1:]))
